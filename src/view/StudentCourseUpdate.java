...
public class StudentCourseUpdate extends JFrame{
	/**
	 * 
	 */
	...
	public JLabel getTopLabel() {
		if(topLabel == null){
			topLabel = new JLabel("修 改 选 课 信 息",JLabel.CENTER);
		...
		if (studentIdField.getText().equals("") || courseIdField.getText().equals("")) {
			...
			if(!studentDAO.isExist("学号",studentIdField.getText())
				...
				if (studentCourseDAO.isExist("学号", studentIdField.getText(),"课程编号",courseIdField.getText())) {
					option = JOptionPane.showConfirmDialog(this, "确定要修改吗？", "消息",
				...
				if (option == 0) {
					...
					JOptionPane.showMessageDialog(this, "选课信息修改成功！", "消息",
							JOptionPane.INFORMATION_MESSAGE);
				}
			}
	...
	public JPanel getPanel() {
		...
		Box boxv1=Box.createVerticalBox();
		  ...
		  panel.add(getUpdateButton(), BorderLayout.SOUTH);
		return panel;
	}

	public StudentCourseUpdate(){
		setTitle("修改选课信息");
